...
up = [5.12]
num_particles = [800]
rounds = [9]#eggholder 8, 16, 32, 80, 160, 320//schwefel 26, 52, 104, 260, 520, 1040
fitnessTable = [easom]
num_of_iter=100
######################################################
###########Multi-D Measures###########################
...
#         for j in range(1):
        ...
        while exp<num_of_iter:
            ...
            best_position = gwo(fitness, rounds[j], num_particles[k], dim, low[k], up[k])
            err = fitness(best_position)
            if(err < -0.94):
            
                count +=1
            results.append(err)
            print(exp, ' - ',err, ' , count : ' , count)
    ...
    #     dimensions=dim[j]+1
        wb.save('Server-Results/GWO_0_1_'+str(fitness.__name__)+'_'+str(dim+1)+'1.xls')
    #     wb.close()
        print("All saved")
